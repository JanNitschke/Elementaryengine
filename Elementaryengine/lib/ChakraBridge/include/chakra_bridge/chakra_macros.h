...
#define JSC_PROP(name) \
	.property(L#name,[this]{return get_##name();}, [this](const auto &v) { set_##name(v); }) \
// end of macro
#define JSC_PROP_GET(name) \
	.property(L#name,[this]{return get_##name();}) \
// end of macro
...
#define JSC_METHOD(ArgCount,name) \
	.method<ArgCount>(L#name,[this](BOOST_PP_ENUM_PARAMS(ArgCount,const auto &p)) { return name(BOOST_PP_ENUM_PARAMS(ArgCount,p)); }) \
// end of macro
...
#define JSC_DECLARE_PROP(type, name) \
	...
	virtual void set_##name(type v)=0 \
// end of macro

#define JSC_DECLARE_PROP_GET(type,name) \
	virtual type get_##name()=0 \
// end of macro
